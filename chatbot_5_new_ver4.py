...
with col_class:
    st.session_state.student_class = st.selectbox("반", ["1반", "2반"],
                                                   index=0 if st.session_state.student_class == "1반" else 1,
                                                   key="class_select")
with col_num:
...
def evaluate(cur, ans):
    ...
    def has_unit_pattern(text):
        return bool(re.search(r'\b(?:m/s|km/h|m/초|km/시간|m|km|초|s|h|미터|킬로미터|매초|매시간)\b', text or "", flags=re.I))
