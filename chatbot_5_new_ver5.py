...
def check_correct(item, ans):
    ...
    if needs_reason:
        ...
        sys = (
            ...
            "  5. 속력을 구하는 문제에서 속력 계산 결과가 틀리거나 단위가 틀리면 무조건 틀린 것이다.\n"
            "  예시: 정답이 4m/s 인데 40m/s라고 하면 틀림\n"
            "  예시: 속력 단위가 m/s 인데 학생이 km/h라고 하면 틀림\n"
            "\n"
            "**[오답 케이스]**\n"
            ...
            if is_traffic_safety:
                        sys = (
                            "너는 초등 5학년 과학 선생님이다. "
                            "학생 답이 정답과 의미상 같은지 판단해. "
                            "**이 문항은 교통안전 관련이므로 매우 유연하게 채점한다.**\n"
                            "\n"
                            "**[판단 규칙]**\n"
                            "1. **핵심 개념만 포함되면 정답으로 인정** (표현 길이나 완성도 무관):\n"
                            "   - 정답이 길어도, 학생이 핵심 키워드만 맞게 썼으면 정답\n"
                            "   - 같은 개념을 다른 말로 표현하면 모두 정답\n"
                            "   - 원인과 결과 관계가 논리적이면 정답\n"
                            "   예시:\n"
                            "   • 정답: '사고가 날 때 몸이 부딪치는 충격을 줄이기 위해서이다'\n"
                            "     학생: '충격을 흡수한다', '충격을 줄여준다' → 모두 O (핵심 개념 포함)\n"
                            "   • '속력이 빠르면 위험': '멈추기 힘들다', '사고나면 다친다', '제동거리 길다' → 모두 O\n"
                            "   • '안전띠 역할': '몸을 고정', '튕겨나가지 않게', '밀려나가는걸 막는다', '날아가지 않게', '보호' → 모두 O\n"
                            "   • '에어백 역할': '충격 완화', '충격 흡수', '다치지 않게', '보호' → 모두 O\n"
                            "2. 표현이 달라도 의미가 같으면 정답\n"
                            "   예: 정답 '멈추기 어렵다' → 학생 '멈추기 힘들다' (O)\n"
                            "\n"
                            "3. 단, 핵심 내용이 완전히 반대이거나 무관하면 틀린 것\n"
                            "   예: '안전띠 역할' → 학생 '빨리 가게 해준다' (X)\n"
                            "\n"
                            "JSON만 출력: {\"is_correct\": true/false}"
                        )
            else:
...
def evaluate_metacog_answer(objective, question, student_answer):
    ...
    # 교통안전 관련 질문에 대한 특별 지침 추가
    is_traffic_safety = objective == "6과07-03"
    if is_traffic_safety:
        sys += (
            "\n**특별 지침 (교통안전 관련 질문):**\n"
            "- '안전띠', '에어백', '안전장치', '속도 제한', '서행', '과속 금지', '과속방지턱', '표지판' 등을 언급하면 excellent\n"
            "- **1-2개만 언급해도 완벽한 답변임** (추가 답변을 요구하지 말 것!)\n"
            "- **needs_more는 거의 항상 false** (교통안전 관련 답변이 있으면 충분)\n"
            "- 피드백은 칭찬 위주로 (예: '정확해요!', '좋은 예시네요!', '완벽해요!')\n"
            "- 절대로 '다른 방법도 생각해보세요' 같은 추가 요구 금지\n"
        )
